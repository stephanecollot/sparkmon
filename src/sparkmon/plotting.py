...
def plot_db(executors_db: Dict[Any, Any]) -> None:
    ...
    def plot_max_value(ax: plt.Axes, s: Any, string_rep=lambda x: f'{x:0.2f}') -> None:
        ax.annotate(
            string_rep(s.max()), xy=(1, s.max()),
            xytext=(8, 0), xycoords=('axes fraction', 'data'), textcoords='offset points')

    def mmm_plot(ax: plt.Axes, value, executors_db_df, title_prefix, pct=False, ylim=None) -> None:
        ...
        if not pct:
            ...
            ax.set_yticks(ax.get_yticks())  # Necessary to remove UserWarning: FixedFormatter should only be used together with FixedLocator
